...
export default function EmployeeManagement() {
  ...
  const handleEdit = (employee: Employee) => {
    ...
    reset(employee); // 👈 cleaner than multiple setValue calls
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              {[
                ...
                <div key={field.name} className="flex flex-col">
                  ...
                  <Input
                    {...register(field.name as keyof Employee)}
                    placeholder={field.placeholder}
                    className="mt-1 rounded-lg border-gray-300"
                  />
                  {errors[field.name as keyof Employee] && (
                    <p className="text-red-500 text-sm mt-1">
                      {errors[field.name as keyof Employee]?.message as string}
                    </p>
                  )}
                </div>
              ))}
        ...
        {/* Employee List */}
        <Card className="">
          <CardHeader className="bg-[#0d3b66] text-white rounded-t-xl flex flex-wrap gap-4 justify-between items-center px-6 py-4">
            <CardTitle className="text-lg font-semibold">
              Employee List
            </CardTitle>
            ...
            {employees.length === 0 ? (
              ...
              <div className="grid gap-4">
                {employees.map((emp) => (
                  <div
                    key={emp._id}
                    className="flex justify-between items-center p-4 border rounded-xl bg-white hover:shadow-md transition"
                  >
                    <div>
