...
export default function TransactionByHour() {
  ...
  if (isError || !heatmap) return <div className="text-center py-10 text-red-500">Error loading data.</div>;
  ...
  return (
    <Card className="w-full max-w-6xl mx-auto mt-6 sm:mt-10 p-4 sm:p-6 shadow-xl rounded-2xl bg-white">
      <CardHeader>
              ...
              <React.Fragment key={rowIndex}>
                ...
                {row.map((value, colIndex) => {
                  ...
                  return (
                    <motion.div
                      key={colIndex}
                      className="rounded-md text-center text-xs sm:text-sm py-1.5 sm:py-2 font-semibold cursor-pointer transition-all duration-300 hover:scale-105 hover:shadow-md"
                      style={{
                      ...
                      transition={{ delay: rowIndex * 0.07 + colIndex * 0.03 }}
                    >
                      {value}
                    </motion.div>
                  );
